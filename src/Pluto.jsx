...
// Import Pluto texture
import plutoTexture from "./assets/textures/2k_ceres_fictional.jpg";

// Simple Pluto component
function SimplePluto() {
  const plutoRef = useRef();
  const plutoTex = useLoader(TextureLoader, plutoTexture);

  useFrame((state) => {
    const time = state.clock.elapsedTime;
    if (plutoRef.current) {
      // Pluto rotation (6.4 Earth days = 1 Pluto day)
      plutoRef.current.rotation.y += 0.002;

      // Subtle wobble
      plutoRef.current.rotation.x = Math.sin(time * 0.1) * 0.01;
    }
  });
...
// Simple lighting for Pluto
function PlutoLighting() {
  return (
    <>
      <directionalLight
        position={[15, 8, 10]}
...
// Main Pluto component
export default function Pluto({ onReturnToSolarSystem }) {
